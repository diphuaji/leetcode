...
class Graph{
    public:
        ...
        void dfs();
        void dfsv2();
        void dfsRcr(Vertex &,const shared_ptr<int>);
...
void Graph::dfsv2(){
    this->reset();
    shared_ptr<int> time=make_shared<int>();
    *time=0;
    for(int i=0;i<this->vList.size();i++){
        Vertex & v=this->vList[i];
        if(v.c!=white) continue;
        v.dt=++(*time);
        this->dfsRcr(v,time);
    }
}

void Graph::dfsRcr(Vertex & v,const shared_ptr<int> time){ 
    for(int i=0;i<v.adjList.size();i++){
        Vertex * vtx=v.adjList[i];
        if(vtx->c!=white) 
            continue;
        vtx->c=gray;
        vtx->dt=++(*time);
        vtx->p=&v;
        this->dfsRcr(*vtx,time);
    }
    v.c=black;
    v.ft=++(*time);
    cout<<"id: "<<v.id<<endl;
